...
impl Row {
    ...
    pub fn resize(&mut self, len: u16, cell: crate::cell::Cell) {
        let was_longer = self.cells.len() > usize::from(len);
        self.cells.resize(usize::from(len), cell);
        self.wrapped = false;
        // When shrinking, the cell that becomes the new last column may have
        // been the first half of a wide character whose continuation just
        // got truncated. Leaving it as-is causes Screen::text() to follow the
        // is_wide() branch and unwrap drawing_cell_mut(col + 1) → None.
        // Mirror Row::truncate's cleanup so the grid stays consistent.
        if was_longer && len > 0 {
            let last_cell = &mut self.cells[usize::from(len) - 1];
            if last_cell.is_wide() {
                last_cell.clear(*last_cell.attrs());
